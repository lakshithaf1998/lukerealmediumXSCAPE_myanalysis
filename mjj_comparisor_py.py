...
#   • ADD: Apply the configured bounds verbatim instead of data-driven auto-scaling, and warn in comparisor_py.log when plotted ratio points fall outside the configured window so sparse tails are easy to retune.
...
#   • ADD: Track every saved comparison figure and every placeholder figure, then print an end-of-run exact summary of real plots vs placeholders (including per-output bucket counts) into comparisor_py.log / SLURM stdout.
...
#   • FIX: Resolve the generic selected-dijet denominator for these jet-pT overlays from the published dphi_all_per_R counts, which track the base accepted dijet-pair selection without extra Mjj/xJ/AJ-specific gates.
...
#   • FIX: Buffer early _log() messages until compare_dir/logs/comparisor_py.log is opened, then flush them into the standard comparator log so startup/fatal diagnostics are not lost.
...
#   • FIX: Rename the main comparisor log to compare_dir/logs/comparisor_py.log so it is no longer mislabeled as fit_py.log.
...
#   • FIX: Apply plot styling only after compare_dir/logs/comparisor_py.log is open, so resolved font settings and any fallback warnings are captured in the comparisor log.
...
# 2026-03-03: FIX (v4.43): Comparisor logs now go to compare_dir/logs/comparisor_py.log (and create logs/ if missing), instead of writing directly into compare_dir.
...
# These are used ONLY when we compare exactly two runs (vacuum vs brick).
# Multi-curve overlays intentionally keep their existing palettes.
COMPARE_COLOR_A = '#0072FF'  # bright blue
COMPARE_COLOR_B = '#E60000'  # bright red
...
def _suptitle_rect_top(title: str, top_single: float = 0.97, top_wrapped: float = 0.93) -> float:
    return top_wrapped if ('\n' in str(title or '')) else top_single
...
DEFAULT_RATIO_MODE = "med"  # 'med' => brick/vacuum, 'inv' => vacuum/brick
...
def apply_plot_style(cfg: Dict[str, str]) -> Dict[str, float]:
    """Apply matplotlib font/style knobs from jetscape.ini and return resolved sizes."""
...
def _add_jetr_uncertainty_note(ax) -> None:
    ax.text(
        ...
        transform=ax.transAxes, ha='right', va='bottom', fontsize=_FONTS['note'],
...
def _add_autozoom_note(ax) -> None:
    ax.text(
        0.02, 0.02,
        'display x-range auto-zoomed\nto populated bins',
        transform=ax.transAxes, ha='left', va='bottom', fontsize=_FONTS['note'],
        bbox=dict(boxstyle='round,pad=0.25', facecolor='white', alpha=0.85, edgecolor='black')
    )
...
def overlay_plot(
    ...
    *,
    ...
    if tag:
        ...
        wrapped_title = _set_wrapped_ax_title(
            ax,
            f"Dijet invariant mass overlay  [{lead_pt_lbl}; R={R:.2f}{mjj_bb_tag}{ystar_title_suffix}]",
        )
    else:
        wrapped_title = _set_wrapped_ax_title(ax, f"Dijet invariant mass overlay  [{hn}]")
...
def overlay_plot_perdijet(
    ...
    *,
    ...
    """
    Per-dijet normalized overlay:
      y -> y / σ_sel   where σ_sel = ∑ (y_i * Δx_i)
    Units become 1/GeV instead of mb/GeV.

    This keeps shape differences visible even if the total cross section differs.
    """
    xcA, yA, eA, bwA, entA = A
    xcB, yB, eB, bwB, entB = B

    out_path = os.path.join(out_dir, f"{hn}_comparisor_perdijet.png")
    if SINGLE_SLICE_MODE and (countA is not None or countB is not None):
    ...
    if tag:
        ...
        wrapped_title = _set_wrapped_ax_title(ax, f"Dijet invariant mass overlay (per selection normalized)  [{hn}]")
...
def combined_all_plot(
    ...
    markers = ["o", "^", "s", "D", "v", "P"]
    colorsA = ["tab:blue", "tab:green", "tab:purple", "tab:cyan", "tab:olive", "tab:pink"]
    colorsB = ["tab:orange", "tab:red", "tab:brown", "tab:gray", "black", "gold"]
    fig, ax = plt.subplots(figsize=(9, 7))
    ax.set_xscale("log")
    ax.set_yscale("log")
    ...
    ax.legend(loc="best", fontsize=_FONTS['legend'], frameon=True)
...
def plot_particle_overlay(
    ...
    *,
    ...
    if kind == 'pt':
        ...
        _set_wrapped_ax_title(ax, "Inclusive weighted yield of accepted particle-level constituents (pre-clustering): $p_T$ comparison", max_chars=90)
    elif kind == 'eta':
        ...
        _set_wrapped_ax_title(ax, r"Inclusive weighted yield of accepted particle-level constituents (pre-clustering): pseudorapidity $\eta$ comparison", max_chars=90)
    else:
        mV = np.isfinite(xcV) & np.isfinite(yV) & np.isfinite(eV_plot)
        mM = np.isfinite(xcM) & np.isfinite(yM) & np.isfinite(eM_plot)
        if not (np.any(mV) or np.any(mM)):
        ...
        _set_wrapped_ax_title(ax, r"Inclusive weighted yield of accepted particle-level constituents (pre-clustering): azimuth $\phi$ comparison", max_chars=90)
...
def plot_sublead_vs_lead_h2_compare(
    ...
    *,
    ...
    _set_wrapped_ax_title(axV, f"{label_vac} {pair_tag}", max_chars=54, pad_single=5.0, pad_wrapped=8.5)
    _set_wrapped_ax_title(axM, f"{label_med} {pair_tag}", max_chars=54, pad_single=5.0, pad_wrapped=8.5)
    ...
    if use_ratio:
        ...
        _set_wrapped_ax_title(axR, f"Ratio: {ratio_title}", max_chars=48, pad_single=5.0, pad_wrapped=8.0)
    else:
    ...
    if autozoom_note:
        for ax in (axV, axM, axR):
            ax.text(0.03, 0.03, 'display x/y range auto-zoomed\nto populated bins', transform=ax.transAxes, ha='left', va='bottom', fontsize=_FONTS['note'], bbox=dict(boxstyle='round,pad=0.25', facecolor='white', alpha=0.78, edgecolor='black'))

    wrapped_suptitle = _set_wrapped_suptitle(fig, 'Subleading vs leading jet $p_T$ comparison map' + pair_tag, y=0.985, max_chars=92)
    ...
    *,
    ...
    if use_perdijet:
        ...
        if yA_plot is None or eA_plot is None or yB_plot is None or eB_plot is None:
            _log(f"[WARN] jetpt {kind} R={R:.2f}: per-selection normalization failed; skipping overlay")
            return
    else:
        sigA = sigB = float('nan')
    ...
    wrapped_title = _set_wrapped_ax_title(ax, title_core, max_chars=(82 if is_inclusive else 72), pad_single=6.0, pad_wrapped=11.0)
...
def plot_jetpt_ratio(
    ...
    *,
    ...
    wrapped_title = _set_wrapped_ax_title(ax, f"Jet spectrum ratio ({kind})" + title_suffix, max_chars=(82 if kind == 'incl' else 72), pad_single=6.0, pad_wrapped=11.0)
    _add_autozoom_note(ax)
    ax.legend(loc="best", frameon=True)
...
def plot_dphi_stacked(
    ...
    *,
    ...
    If use_perdijet=True, each distribution is normalized by its own σ_sel so the plots show *shape* only.
    ...
    ax3.text(0.02, 0.88, "med/vac" if ratio_mode != 'inv' else "vac/med", transform=ax3.transAxes, fontsize=_FONTS['note'])
    if ratio_rebin_note:
        ax3.text(0.98, 0.88, ratio_rebin_note, transform=ax3.transAxes, ha='right', va='top', fontsize=_FONTS['note'], bbox=dict(boxstyle='round,pad=0.25', facecolor='white', alpha=0.80, edgecolor='black'))
    ax3.set_xlabel((r"$|\Delta\varphi|$ (rad), folded to $[0,\pi]$" if hn.startswith("dphi_abs_") else r"$\Delta\varphi$ (rad)"))
    ax3.set_ylabel('ratio')

    base_title = r"Dijet azimuthal decorrelation $|\Delta\varphi|$ comparison (folded to $[0,\pi]$)" if hn.startswith("dphi_abs_") else r"Dijet azimuthal decorrelation $\Delta\varphi$ comparison ($0\leq \Delta\varphi < 2\pi$)"
    ...
    if tag:
        ...
        wrapped_suptitle = _set_wrapped_suptitle(fig, f"{base_title} ({suffix})  [R={Rv:.2f}; lead pT {pt_lbl}]{sel_part}", y=0.98, max_chars=106)
    else:
        sel_txt = _dphi_selection_text(ini, float('nan'), hn)
        sel_part = f"  [{sel_txt}]" if sel_txt else ""
        wrapped_suptitle = _set_wrapped_suptitle(fig, f"{base_title} ({suffix}): {hn}{sel_part}", y=0.98, max_chars=106)
...
def plot_imbalance_stacked(
    ...
    *,
    ...
    - If use_perdijet=True, each distribution is normalized by its own σ_sel (shape-only) when possible.
    ...
    if not use_perdijet:
        ...
        if Rv is not None:
            wrapped_suptitle = _set_wrapped_suptitle(fig, f"{obs} comparison (absolute overlay, {scale_tag})" + _pair_title_bracket(Rv, hn, include_ystar=True, include_ptlead=True, include_imbalance_backtoback=True), y=0.98, max_chars=108)
        else:
            wrapped_suptitle = _set_wrapped_suptitle(fig, f"{obs} comparison (absolute overlay, {scale_tag}): {hn}{' [nosubfrac]' if hn.endswith('_nosubfrac') else ''}", y=0.98, max_chars=108)
    ...
    if Rv is not None:
        wrapped_suptitle = _set_wrapped_suptitle(fig, f"{obs} comparison ({suffix}, {scale_tag})" + _pair_title_bracket(Rv, hn, include_ystar=True, include_ptlead=True, include_imbalance_backtoback=True), y=0.98, max_chars=108)
    else:
        wrapped_suptitle = _set_wrapped_suptitle(fig, f"{obs} comparison ({suffix}, {scale_tag}): {hn}{' [nosubfrac]' if hn.endswith('_nosubfrac') else ''}", y=0.98, max_chars=108)
...
def plot_jetR_ratio_compare(
    ...
    *,
    ...
    for r_num, r_den in JET_RADIUS_COMPARE_PAIRS:
        ...
        _set_wrapped_ax_title(ax, f"Jet radius dependence ({kind}){branch_tag}: {pair_label}")
        _add_autozoom_note(ax)
        ...
        if len(xcA) == len(xcB) and np.allclose(xcA, xcB, rtol=0, atol=1e-9):
            ...
            if np.any(m_plot):
                ...
                _set_wrapped_ax_title(ax, f"Double ratio of jet-R dependence ({kind}){branch_tag}: {pair_label}")
                _add_autozoom_note(ax)
                _add_jetr_uncertainty_note(ax)
                ax.legend(loc='best', frameon=True)
                fig.tight_layout()
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=["med", "inv"],
        default=None,
        help="med: brick/vacuum (medium/reference). inv: vacuum/brick.",
    )
    parser.add_argument(
        "--ms",
        type=float,
    ...
    global INCLUDE_XSEC_NORM_ERR, PERDIJET_COV_MODE, COMBINED_R_LIST, JET_RADIUS_COMPARE_PAIRS, SINGLE_SLICE_MODE, SELECTED_DIJET_COUNTS_A, SELECTED_DIJET_COUNTS_B, JET_PT_MAX_CFG, DPHI_RATIO_REBIN_FACTOR, DPHI_ABS_RATIO_REBIN_FACTOR, RATIO_YLIMS
    ...
    dispA = f"{_pretty_label(labelA_raw)} ({_pretty_label(runA)})"
    dispB = f"{_pretty_label(labelB_raw)} ({_pretty_label(runB)})"
    ...
    out_dir_dphi_abs_sub     = os.path.join(plots_abs_sub,  "dphi_fit")
    out_dir_dphi_abs_ns      = os.path.join(plots_abs_ns,   "dphi_fit")
    out_dir_dphi_norm_sub    = os.path.join(plots_norm_sub, "dphi_fit")
    out_dir_dphi_norm_ns     = os.path.join(plots_norm_ns,  "dphi_fit")
    ...
    out_dir_custom = os.path.join(compare_dir, "plots", "custom")

    # Comparisor-only diagnostics that do not exist in fitter.
    out_dir_jet_ratio = os.path.join(out_dir_custom, "jet_spectra_ratio")
    ...
    for d in [
        ...
        out_dir_dphi_abs_sub, out_dir_dphi_abs_ns, out_dir_dphi_norm_sub, out_dir_dphi_norm_ns,
        ...
        out_dir_jetR, out_dir_jetR_sub, out_dir_jetR_ns, out_dir_particle, out_dir_lead_sublead_corr_sub, out_dir_lead_sublead_corr_ns, out_dir_custom, out_dir_jet_ratio,
    ]:
        ensure_dir(d)
    # Logging (fitter-consistent filename): write into compare_dir/logs/comparisor_py.log
    # so both python stages use the same log naming convention.
    logs_dir = os.path.join(compare_dir, "logs")
    os.makedirs(logs_dir, exist_ok=True)
    log_path = os.path.join(logs_dir, "comparisor_py.log")
    _set_log_file(log_path, mode="a")

    # Plot styling knobs (optional) from jetscape.ini. Apply after log setup so
    # any fallback warnings and resolved values land in comparisor_py.log.
    global _FONTS
    _FONTS = apply_plot_style(cfg)

    _log("stage: merged-observable comparison (not Mjj-only)")
    ...
    if jA and jB:
        ...
        # Decide which run is "brick" vs "vacuum" once, for ratio convention
        ...
        for kind, R, is_ns in keys:
            ...
            out_dir_jet_ratio_local = out_dir_jet_ratio if kind == 'incl' else os.path.join(out_dir_jet_ratio, 'nosubfrac' if branch_ns else 'subfrac')
            ensure_dir(out_dir_jet_ratio_local)
            ...
            if num is A:
                hn_num = hnA
                xsec_band_num = XSEC_BAND_A
            else:
                hn_num = hnB
                xsec_band_num = XSEC_BAND_B
            if den is A:
                hn_den = hnA
                xsec_band_den = XSEC_BAND_A
            else:
                hn_den = hnB
                xsec_band_den = XSEC_BAND_B
            ...
            plot_jetpt_ratio(
                ...
                denominator=den,
                ratio_label=rlab,
                out_dir=out_dir_jet_ratio_local,
                ms=ms,
                hn_num=hn_num,
                hn_den=hn_den,
                xsec_band_num=xsec_band_num,
                xsec_band_den=xsec_band_den,
            )
...
# ------------------------- Δφ comparisons (stacked, no messy overlays) -------------------------
    _log("Reading Δφ histograms...")
    dA = read_dphi_hists(rootA)
    dB = read_dphi_hists(rootB)
    _log(f"Δφ hists: A={len(dA)}  B={len(dB)}")

    # decide vacuum vs brick for panel ordering
    A_is_vac = _is_vacuum(runA)
    B_is_vac = _is_vacuum(runB)
    A_is_brick = _is_brick(runA)
    B_is_brick = _is_brick(runB)

    # choose which dataset is vacuum/brick for stacked layout
    if A_is_vac and B_is_brick:
        ...
        # fallback: keep A on top, B on bottom
        vac_tag, brick_tag = 'A', 'B'
        vac_h, brick_h = dA, dB
        lab_v, lab_b = dispA, dispB

    # compare only the 'all' hists by default (cleanest)
    names = sorted(set(vac_h.keys()) & set(brick_h.keys()))
    for hn in names:
        tag = parse_dphi_name(hn)
        if not tag:
            continue
        Rv, lo, hi, isAll = tag
        is_ns = hn.endswith('_nosubfrac')
        out_dir_dphi_norm_local = out_dir_dphi_norm_ns if is_ns else out_dir_dphi_norm_sub
        out_dir_dphi_abs_local  = out_dir_dphi_abs_ns if is_ns else out_dir_dphi_abs_sub
        countA = countB = None
        if SINGLE_SLICE_MODE:
            try:
                countA = SELECTED_DIJET_COUNTS_A.count_for("dphi", Rv, is_nosubfrac=is_ns, ptlo=(None if isAll else lo), pthi=(None if isAll else hi), is_abs=False)
                countB = SELECTED_DIJET_COUNTS_B.count_for("dphi", Rv, is_nosubfrac=is_ns, ptlo=(None if isAll else lo), pthi=(None if isAll else hi), is_abs=False)
                if vac_tag == 'B':
        ...
        plot_dphi_stacked(
            ...
            label_vac=lab_v,
            label_brick=lab_b,
            out_dir=out_dir_dphi_norm_local,
        ...
        plot_dphi_stacked(
            ...
            label_vac=lab_v,
            label_brick=lab_b,
            out_dir=out_dir_dphi_abs_local,
            xsec_band_vac=(XSEC_BAND_A if vac_tag == 'A' else XSEC_BAND_B),
            xsec_band_brick=(XSEC_BAND_B if vac_tag == 'A' else XSEC_BAND_A),
            ini=cfg,
            ratio_mode=args.ratio_mode,
    ...
    # Use the same vacuum/brick mapping chosen above (vac_tag).
    if vac_tag == 'A':

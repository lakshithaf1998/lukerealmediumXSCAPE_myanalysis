#!/usr/bin/env bash
# submit_mjj_comparisor.sh  v2.1
# CHANGELOG
# v2.1 (strict compare ratio-range preflight):
#   • NEW: Require all compare-side ratio-panel y-range knobs in jetscape.ini before sbatch: MJJ/JETPT/DPHI/DPHI_ABS/XJ/AJ/JETR/LEAD_SUBLEAD_PROFILE lower+upper bounds.
#   • FIX: Validate each configured ratio window as finite, ordered, and containing unity so bad compare settings fail on the login node instead of wasting queue slots.
# v2.0 (single-slice selected-dijet preflight):
#   • NEW: When SINGLE_SLICE_MODE=1, validate each run's merged selected_dijet_counts_summary metadata before sbatch.
#   • NEW: Require the published summary path (or canonical fallback) to exist, merged_slice_count=1, count_semantics=event_level_selected_dijet_pair_counts,
#     and the required per-R / per-pT count arrays to match JET_RADIUS_LIST and PT_LEAD_BIN_EDGES.
#   • FIX: Fail on invalid SINGLE_SLICE_MODE or incompatible single-slice ini arrays before consuming queue slots.
# v1.9 (comparison log prefix cleanup):
#   • FIX: Rename submitted SLURM stdout/stderr files from mjj-comp-%j.{out,err} to compare-%j.{out,err} so the wrapper log naming matches the full comparison-stage scope.
#   • SAFETY: Wrapper filename, sbatch target, compare-tag parsing, and output directories are unchanged.
# v1.8 (scope wording + honest comparison logging):
#   • FIX: Clarify that this wrapper submits the full merged-observable comparison stage, not just Mjj plots.
#   • FIX: Submission/error log text now says comparison job/stage while keeping the existing wrapper filename for workflow compatibility.
#   • SAFETY: sbatch target, compare-tag parsing, and output locations are unchanged.
# v1.7 (submit-side python fallback):
#   • FIX: Resolve a submit-side Python interpreter as python3 or python before wrapper preflight, so sbatch submission does not fail just because only one name exists on the login environment.
#   • SAFETY: Compute-node module loading and python_mjj_comparisor.slurm behavior are unchanged.
# v1.6 (ini-driven mail settings):
...
#   • FIX: Keep python_mjj_comparisor.slurm free of hardcoded mail settings so jetscape.ini is the only mail source of truth.
# v1.5 (shell preflight case fix):
...
# v1.4 (strict xsec preflight):
#   • NEW: Add wrapper-side validation of each run's final/xsec_summary.json when INCLUDE_XSEC_NORM_ERR=1.
#   • NEW: Require each compared run to have final/dphi_allSlices.root, finite positive xsec_total_rel_err, a non-empty
#     slices list, finite positive per-slice xsec_rel_err, and every referenced per-slice rootfile before sbatch.
#   • FIX: Keep wrapper preflight consistent with mjj_comparisor_py.py so broken xsec contracts fail before consuming queue slots.
# v1.3 (slurm submission hardening):
#   • Submit with sbatch --parsable so the comparisor gets a machine-readable job id.
...
#   • NEW: Read compare configuration from jetscape.ini (single source of truth):
#       - COMPARE_RUN_TAG_A / COMPARE_RUN_TAG_B
#       - COMPARE_LABEL_A   / COMPARE_LABEL_B
#       - COMPARE_RATIO_MODE (med|inv)
#     Env vars may still override for convenience (COMP_RUN_TAG_A/B, COMP_LABEL_A/B, COMP_RATIO_MODE),
#     but overrides are logged loudly.
#   • SAFETY: Fail fast if tags are missing or if the corresponding run folders do not exist.
#   • DIAG: On failure, prints RUNS_ROOT and a short list of available run tags.
#
# v1.1 (repro knobs):
#   • DIAG: Echo ini-configurable knobs used by the comparisor (COMBINE_R_LIST, INCLUDE_XSEC_NORM_ERR, COVARIANCE_MODE).
#
# v1.0 (major):
#   • NEW: Submits python_mjj_comparisor.slurm to compare merged observables from two run-tags (vacuum vs brick/medium).
#   • Sends logs to: $HOME/xscape_runs/comparisons/<TAG_A>_vs_<TAG_B>/logs/
#   • Supports overriding tags via env vars:
#       COMP_RUN_TAG_A=... COMP_RUN_TAG_B=... ./submit_mjj_comparisor.sh
...
log "[submit] Submitting merged-observable comparison job..."
SBATCH_OUT="$(
  sbatch --parsable \
    --chdir="${BASE_DIR}" \
    --export=ALL,COMP_RUN_TAG_A="${RUN_TAG_A}",COMP_RUN_TAG_B="${RUN_TAG_B}",COMP_LABEL_A="${LABEL_A}",COMP_LABEL_B="${LABEL_B}",COMP_RATIO_MODE="${RATIO_MODE}" \
    --output="${LOG_DIR}/compare-%j.out" \
    --error="${LOG_DIR}/compare-%j.err" \
    "${MAIL_ARGS[@]}" \
    python_mjj_comparisor.slurm
)" || die "sbatch failed for comparison job"
...
log "[submit] Logs:  ${LOG_DIR}/compare-${JID}.out  and  ${LOG_DIR}/compare-${JID}.err"
